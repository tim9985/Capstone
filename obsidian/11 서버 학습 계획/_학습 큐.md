...
> 실행: `drone_yolo/chain_queue.sh` · 진행 로그 `logs/QUEUE.md` · 결과 `metrics/AUTO_RESULT.md`
> 각 단계는 **실패해도 다음으로 넘어간다** (GPU 를 쉬게 두지 않는다)

## 2026-09-24 — 기본 마운트각 60° 반영

> [!important] 계획 기준: [[03 학습 계획 v6 (60° 기준)]]
> · 진행 중 `p2m_place` 는 그대로 완주 (test_v2 + **test_obl** 로 판정)
> · 다음 `rect_place` 는 **제외** → 기준선 재측정(test_obl·test_kr) → `v6_obl`
> · 합성 데이터 보류
