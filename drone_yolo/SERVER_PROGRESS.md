...
- best.pt 는 fitness 로 골라진다. **설치본 8.4.102 의 fitness = mAP50-95 만** (09-15 확인 · 이 문서의 fitness 수치 0.3816 등은 `train_status.py` 옛 공식 0.1·mAP50 + 0.9·mAP50-95 값). 목표는 AP50.
- `auto_augment` · `erasing` 은 분류 전용이라 탐지에는 영향 없음.

**결론**: 학습 설정으로는 0.65 를 못 넘는다 → **평가셋 재정의 (가시도별 · 시간 분할 제외 · 장소 분리 · 음성) → 하향 숲 데이터 보강**.

## 4-4. 화각 · 고도 비교 실험 (09-15 ~ 16 · 진행 중)

사용자 결정: 렌즈 수평 화각 54 · 65 · 80 · 90° × 고도 10 · 20 · 25 · 30 · 40 m × 서 있는 / 누운 사람 재현율 표 → 예산안 렌즈 결정 근거.
볼트 `04 실험/화각·고도 비교 실험` · 합의안 `05 결정/결정 - 하드웨어 예산안 합의 v4`.

| 항목 | 값 |
|---|---|
| 대기열 | `train_fov.sh` (09-15 07:36 UTC) · 부팅 입구 `train_fov_boot.sh` · 백업 · 단계 전환 `fov_guard.sh` · 상태 `python fov_status.py` |
| 학습 데이터 | `data/det_fov` — NOMAD 60명 a10 · a30 · a50 · a70 + WiSARD 전체 · 사람 16~160 px 로그 균등 · train 30,056 / val 7,692 · 분할 기존과 같음 확인 · SARD 제외 |
| 시험셋 | `data/det_fov_test` v4 (`make_fov_testsets.py`) — val 원본을 칸마다 화면 중앙 사람 px 로 맞춘 34칸 · 원본 세트 s10 1,284 · s20 1,631 · l10 303 · l20 687장 · 거울 바둑판 채움 · 채점 orig & !seam & near_ref · 세트 안 교집합 평균 (v1 은 `det_fov_test_v1` 보관) |
| 학습 1 | yolo11s · 1280 · batch 16 · 30에폭 · SGD lr0 0.01 · scale 0.3 · close_mosaic 5 · 에폭당 13.3분 |
| 학습 2 | yolo11m · 1280 · batch 8 · 시간 제한 8시간 (20시간 안에 끝내려고 10 → 8 — 실행 중 스크립트를 같은 inode · 같은 바이트 길이로 수정) |
| 평가 | `eval_fov.py` → `metrics/eval_fov_<실행>.csv` · 재현율@0.15 (주) · AP50 (보조) · M1 함께 |

진행 (09-15 13:31 UTC): yolo11s 22/30에폭 · val(16~160 px) AP50 0.420 · AP50-95 0.220 · 재현율 0.394 · val box 손실 2.32 → 1.99 계속 감소 · GPU 소실 0회.
예상: yolo11s 끝 15:12 UTC → 평가 2개 → 첫 결과표 16:00 UTC 무렵 → yolo11m 8시간 → 최종 결과표 09-16 00:40 UTC 무렵.

**1차 결과 (09-15 15:51 UTC · yolo11s 30에폭 6.8시간 · best 25에폭)** — `metrics/eval_fov_fov_11s_1280_all.csv` · `eval_fov_m1_11m_1280.csv`

| 재현율@0.15 (20~40 m 세트 · 행은 자세가 아니라 **0.5 m / 1.7 m 크기 가정**) | 54° | 65° | 80° | 90° |
|---|---|---|---|---|
| 서 있음 20 m | 47 px 0.34 | 38 px 0.28 | 29 px 0.23 | 24 px 0.19 |
| 서 있음 25 m | 38 px **0.28** | 30 px **0.23** | 23 px **0.19** | 19 px **0.16** |
| 서 있음 30 m | 31 px 0.24 | 25 px 0.20 | 19 px 0.16 | 16 px 0.12 |
| 서 있음 40 m | 24 px 0.19 | 19 px 0.16 | 14 px* 0.09 | 12 px* 0.05 |
| 누움 25 m | 128 px 0.63 | 102 px 0.61 | 78 px 0.54 | 65 px 0.48 |
| 누움 40 m | 80 px 0.54 | 64 px 0.47 | 49 px 0.39 | 41 px 0.32 |

- 같은 px 면 같은 값 → 탐지율은 사람 px 로만 정해진다. 25 m · 54° 대비 서 있음 65° −18 % · 80° −32 % · 90° −43 % · 누움 −3 · −14 · −24 %.
- M1 (94 px × 지터 학습) 은 40 px 아래에서 붕괴: 31 px 0.08 · 24 px 0.02 · 19 px 이하 0.00 (yolo11s 0.24 · 0.19 · 0.16). 누움 80~128 px 는 M1 0.59~0.66 이 조금 높다.
- 절대값이 낮다 (서 있음 47 px 0.34 · 누움 160 px 0.64): 비스듬한 전신을 어깨 폭 크기로 줄인 흉내 · 가시도 <30 포함 (≥30 만 보면 +0.05~0.06) · WiSARD 가 NOMAD 의 1/3 (38 px 0.13 vs 0.40).
- 10 m 누움 320 px(외삽) 0.35 — 학습 범위를 넘는 큰 사람도 무너진다.
- yolo11m: 09-16 08:17 KST 18/19에폭 · val AP50 최고 10에폭 0.478 (yolo11s 0.422) 뒤 0.43~0.44 · 최종 표 09:30 KST 무렵.

| 시험셋 판 | 짚은 문제 (로컬 세션 검토) | 고친 것 |
|---|---|---|
| v1 | 작은 칸 회색 여백 최대 85 % → 배경 방해물이 사라져 점수 부풀림 · 한 조건 원본이라 쉬운 장면만 (FHL 사용률 18 %) | — |
| v2 · v3 | — | 거울 바둑판 · 자세별 원본 조건 · 복제 칸 라벨 + meta(orig / seam / vis) · 이미지 평균 재현율 · 고도 10 · 40 m 추가 |
| v4 | 10 m 칸(누움 320 px)이 원본 조건을 끌어올려 누움 원본이 NOMAD a10 + SuddenValley 로 쏠림 · seam 제외로 칸마다 채점 원본이 달라짐 | 원본 세트 = 자세 × {10 m, 20~40 m} · 세트 안 교집합 평균 · near_ref (기준 인물 ±30 %) |

- 한계: 크기만 흉내 (렌즈 왜곡 · 가장자리 옆모습 · 하향 모습 없음 → 80~90° 낙관적) · 학습 16~160 px 밖 칸은 외삽 · 시험 원본 = best 선택용 val 과 같은 배우 · 비행 → 상대 비교용 · 10 m 행은 원본 세트가 달라 직접 비교 금지.
- 체크포인트 백업 공백: autoheal 은 부팅 때 읽은 JOB_NAME 으로만 백업 · 복구 → `fov_guard.sh` 가 30초마다 last_backup.pt 복사 · JOB_NAME 을 단계 이름으로 전환, 재부팅 뒤엔 `train_fov_boot.sh` 가 guard 를 다시 켠다.
- 1280×720 조각 학습 · 1920 전체 프레임 추론: 조각은 원래 배율이라 사람 px 가 같다. 창 밖 배경이 적어 프레임당 오탐은 학습 뒤 1920 전체 프레임으로 따로 확인.
